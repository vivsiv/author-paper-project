...
def author_year_features(paper_join, train_out):
	...
	#ays_std = author_years.std().rename("std_pub_year")
...
def author_paper_features(author_join, paper_join, train_out):
	...
	author_journal_groups = author_paper["journal_id"].agg({"journal":(lambda group: list(group))})
	...
	author_journal_groups["journal_count"] = author_journal_groups.apply(lambda row: np.count_nonzero(row["journal"]), axis=1)

	author_paper_features = paper_join[["paper_id", "author_id"]].copy()

	author_paper_features = pd.merge(pd.merge(pd.merge(author_paper_features, author_paper_groups[["author_id", "paper_count"]], how="left", on="author_id"), author_conf_groups[["author_id", "conf_count"]], how="left", on="author_id"), author_journal_groups[["author_id", "journal_count"]], how="left", on="author_id")
