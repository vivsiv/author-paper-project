...
def grid_search(train_data,features,cv_folds=5):
	models = {
	    "forest":(
...
def evaluate(predictions):
	...
	solution_compare = predictions.merge(solution, how="left", on=["author_id","paper_id"])
	solution_compare["wrote_paper_actual"] = solution_compare["wrote_paper_actual"].fillna(0).astype(int)
...
def main():
	...
	print "Reading in Predict Data for {0}".format(base)
	...
	# Uncomment if you want to compare models and get the best one	
	# model = get_best_model(train_data, features)

	model = en.RandomForestClassifier(n_estimators=50)

	# Uncomment if you want to do cv on this model
	# print "Testing model..."
	# cv_folds = 5
	# scores = ms.cross_val_score(model, train_data[features], train_data["wrote_paper"], cv=cv_folds)
	# print "Average Score over %d folds is %0.2f (+/- %0.2f)" % (cv_folds,scores.mean(),scores.std() * 2)
